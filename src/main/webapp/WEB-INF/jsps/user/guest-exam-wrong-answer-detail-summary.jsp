 ...
 <!-- PRELOADER -->
  ...
  <section style="padding-top: 0px;padding-bottom:10px;background-color: white;" id="quizz-intro-section" class="quizz-intro-section learn-section">
        <div class="container">
        				 ...
        				 Test  Summary & Wrong Attempted Questions:-</h3>
        				<hr/>
                ...
                <img src="${pageContext.request.contextPath}/images/icon/writing-icon.png" style="height: 40px;"/>
						<span style="background-color: #ff0000c9;margin-left: 20px;">
						   Wrong Answer (${fn:length(questionList)})
						</span>	
						<span style="background-color: #03dfff30;margin-left: 20px;">
						  Attempted Question (${testConfiguration.totalMarks-totalUnAttemptedQuestions})
						</span>	
						 	<span style="background-color: #fdf2cdd1;margin-left: 40px;">
						 Not Attempted Question (${totalUnAttemptedQuestions})
						</span>	
						<div style="float: right">
							  <span style="background-color: #0ef37a;">Total Correct Question(s) :</span> <b style="background-color: yellow;">(${testConfiguration.secureMarks})</b>
						</div>
						 <br/>
					...
					<tr>
						<td  colspan=3 height="30" 	<c:if test="${question.selectedOption==null}">
														style="background-color:#fdf2cdd1;color: black;font-size: 16px;"
									</c:if>
							<c:if test="${question.selectedOption!=null}">
														style="background-color:#03dfff30;color: black;font-size: 16px;"
									</c:if>
							>
					...
					<tr>
						...
						<td colspan="2" bordercolor="black"
						 <c:if test="${guest.wrong.answers.show=='yes'}"/>						
						...
						<c:if test="${guest.right.answers.show=='yes'}"/>	
					...
					<tr>
						...
						<td 	
						 <c:if test="${guest.wrong.answers.show=='yes'}">	</c:if>
						...
						<c:if test="${guest.right.answers.show=='yes'}">	</c:if>	 	
							...
							<c:if test="${guest.wrong.answers.show=='yes'}"></c:if><c:if test="${question.selectedOption == question.answerId2}">checked</c:if>
							/>
					...
					<tr>
						...
						<input type="radio"  disabled="disabled"  id="selectedAnswerId3"  name="${question.questionId}" value="${question.answerId3}"  
					      <c:if test="${guest.wrong.answers.show=='yes'}"></c:if><c:if test="${question.selectedOption == question.answerId3}">checked</c:if>
					   />
					...
					<tr>
						...
						<span class="style3"><input type="radio"  disabled="disabled"  id="selectedAnswerId4" name="${question.questionId}" value="${question.answerId4}" 
					     <c:if test="${guest.wrong.answers.show=='yes'}"></c:if> <c:if test="${question.selectedOption == question.answerId4}">checked</c:if>
					   />
					...
					<c:if test="${question.selectedOption == question.answerId5 && question.correctOption == question.answerId5}">	
						...
						<span class="style3"><input type="radio"  disabled="disabled"  id="selectedAnswerId5" name="${question.questionId}" value="${question.answerId5}" 
					        <c:if test="${guest.wrong.answers.show=='yes'}"></c:if> <c:if test="${question.selectedOption == question.answerId5}">checked</c:if>
					    />
					...
					<tr>
					 ...
					 <c:if test="${guest.wrong.answers.show=='yes'}">	</c:if>
						...
						<span class="style3"><input type="radio"  disabled="disabled"  id="selectedAnswerId6" name="${question.questionId}" value="${question.answerId6}" 
					       <c:if test="${guest.wrong.answers.show=='yes'}"></c:if><c:if test="${question.selectedOption == question.answerId6}">checked</c:if>
					    />
					...
					<tr>
						...
						<span class="style3"><input type="radio"  disabled="disabled"  id="selectedAnswerId7" name="${question.questionId}" value="${question.answerId7}" 
					      <c:if test="${guest.wrong.answers.show=='yes'}"></c:if><c:if test="${question.selectedOption == question.answerId7}">checked</c:if>
					    />
